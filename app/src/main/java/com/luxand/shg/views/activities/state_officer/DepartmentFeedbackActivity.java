...
public class DepartmentFeedbackActivity extends BaseActivity {

    private static final String TAG = "DepartmentFeedbackActivity";
    ...
    TextView tv_active_groups_value, tv_barchart_name,tv_title;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.fragment_feedback_department);
        barchart =  findViewById(R.id.barchart);
        barchart1 =  findViewById(R.id.barchart1);
        tv_active_groups_value =  findViewById(R.id.tv_active_groups_value);
        ...
        ivMenuBack.setOnClickListener(view -> {
            if(data.getLabel() != null &&data.getLabel().equalsIgnoreCase("group")){
                tv_barchart_name.setText("Village Wise Feedback");
                getDashboard(DepartmentFeedbackActivity.this,districtId,mandalId,"");
                //districtId =String.valueOf(dataList.get((int)e.getX()).getId());

            }else if(data.getLabel() != null && data.getLabel().equalsIgnoreCase("village")){
                tv_barchart_name.setText("Mandal Wise Feedback");
                //mandalId = String.valueOf(dataList.get((int)e.getX()).getId());
                getDashboard(DepartmentFeedbackActivity.this,districtId,"","");
            }else if(data.getLabel() != null && data.getLabel().equalsIgnoreCase("mandal")){
                tv_barchart_name.setText("District Wise Feedback");
                //villageId = String.valueOf(dataList.get((int)e.getX()).getId());
                getDashboard(DepartmentFeedbackActivity.this,"","","");
        ...
        barchart.setOnChartValueSelectedListener(new OnChartValueSelectedListener() {
            ...
            public void onValueSelected(Entry e, Highlight h) {
                ...
                if (data.getLabel() != null && data.getLabel().equalsIgnoreCase("district")) {
                    ...
                    tv_barchart_name.setText("Village Wise Feedback");
                    villageId = String.valueOf(dataList.get((int) e.getX()).getId());
                    getDashboard( DepartmentFeedbackActivity.this, districtId, mandalId, villageId);
                }
    ...
    private void getDashboard(Context mContext, String district_id, String mandal_id, String village_id) {
        ...
        RetrofitInstance.getInstance(mContext).getRestAdapter()
                .getDepartmentFeedback(district_id, mandal_id, village_id)
    ...
    private void setBarchart() {
        ...
        for (int i = 0; i < analytics_lists.size(); i++) {
            entries_values.add(new BarEntry(i, analytics_lists.get(i).getVideoCount()));
            labels.add(analytics_lists.get(i).getName());
            entries_values2.add(new BarEntry(i, analytics_lists.get(i).getAudioCount()));
        }

         //ArrayList<BarDataSet> barDataSets = new ArrayList<>();
        //ArrayList<BarDataSet> barDataSets2 = new ArrayList<>();

        BarDataSet dataset = new BarDataSet(entries_values, "");
        dataset.setColor(getResources().getColor(R.color.sky_blue));
        BarDataSet dataset2 = new BarDataSet(entries_values2, "");
        dataset2.setColor(getResources().getColor(R.color.black));
        ...
        //barchart.groupBars(0, 0.01f, 0.0f);
    ...
    private void setBarWidth(BarData barData) {
        ...
        if (barData.getDataSets().size() > 1) {
            ...
            Log.d(TAG, "setBarWidth: defaultBarWidth "+defaultBarWidth);
            ...
            int groupCount = analytics_lists.size();
            if (groupCount != -1) {
